...
CREATE TABLE IF NOT EXISTS Invitation (
  ...
  creator_email REFERENCES User (email)
    ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS Answer (
  ...
  question_id                                              NOT NULL REFERENCES Question (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  -- other --
...
CREATE TABLE IF NOT EXISTS Report (
  ...
  author     REFERENCES User (id)
    ON UPDATE CASCADE,
  -- other --
  issue VARCHAR(255)                                 NOT NULL CHECK (issue IN
                                                                     ("out of date", "inappropriate content", "inaccurate inforamtion", "contains mistakes"))
);
